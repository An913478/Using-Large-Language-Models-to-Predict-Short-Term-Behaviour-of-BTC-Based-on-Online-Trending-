"""Merge BTC market features with Claude-extracted LLM features.

Loads base market data and Claude LLM features, performs data cleaning and
...
LLM_FILE = os.path.join(BASE_DIR, "data", "processed", "btc_marketaux_llm_features_claude.parquet")
OUTPUT_FILE = os.path.join(BASE_DIR, "data", "processed", "btc_final_features_with_llm_claude.parquet")
...
def main() -> None:
    """Merge base market features with Claude LLM features and write output parquet."""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Merge market + Claude LLM features and write final parquet.")
    p.add_argument("--base", default=BASE_FEATURES_FILE, help="Base features parquet file")
    p.add_argument("--llm", default=LLM_FILE, help="Claude LLM features parquet file")
