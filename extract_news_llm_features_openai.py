"""Extract daily LLM features from MarketAux daily text using OpenAI.

This module queries an OpenAI model (via the `openai` client library) to
extract structured features from daily news text. It writes periodic
checkpoints so the process can be resumed.

This script performs network calls and is NOT marked as smoke-test safe.
"""
...
PROVIDER = "openai"
MODEL_NAME = os.getenv("OPENAI_LLM_COMPARE_MODEL", "gpt-4.1-mini")
MAX_ATTEMPTS = 4
CHECKPOINT_EVERY = 25


def call_openai(client: OpenAI, date_str: str, daily_text: str, model_name: str) -> Dict:
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    """Parse CLI arguments for OpenAI LLM feature extraction."""
    p = argparse.ArgumentParser(description="Extract LLM features using OpenAI")
...
def main(argv: Optional[list[str]] = None) -> None:
    """Entry point for the OpenAI provider CLI."""
    ...
    for i, (_, row) in enumerate(todo_df.iterrows(), start=1):
        ...
        if should_skip_text(daily_text):
            ...
            features = call_openai(client, date_str, daily_text, args.model)
