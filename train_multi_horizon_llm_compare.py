"""Compare multi-horizon LSTM training across OpenAI, Claude, and Gemini LLM providers.

Loads provider-specific LLM feature datasets, trains baseline LSTM models on each,
performs walk-forward validation, and exports fold metrics, summary tables, and
predictions for multi-provider comparison.
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results", "llm_comparison")

DATASET_MAP = {
    "openai": os.path.join(DATA_DIR, "btc_final_features_with_llm_openai.parquet"),
    "claude": os.path.join(DATA_DIR, "btc_final_features_with_llm_claude.parquet"),
    "gemini": os.path.join(DATA_DIR, "btc_final_features_with_llm_gemini.parquet"),
}
...
def run_provider_horizon(
    ...
    logger.info(f"\n==================== PROVIDER {provider.upper()} | HORIZON {horizon_name} ====================")
    ...
    logger.info(f"Sequence count for {provider} {horizon_name}: {len(X_seq)}")

    splits = get_walk_forward_splits(len(X_seq))
    logger.info(f"Number of walk-forward folds for {provider} {horizon_name}: {len(splits)}")

    if not splits:
        raise ValueError(f"No valid walk-forward splits generated for provider={provider}, horizon={horizon_name}.")
    ...
    for fold_idx, (train_end, test_end) in enumerate(splits, start=1):
        ...
        logger.info(
            f"    LSTM  -> RMSE: {rmse_lstm:.6f}, MAE: {mae_lstm:.6f}, "
    ...
    summary_df = (
        fold_df.groupby(["provider", "horizon"], as_index=False)
        .agg(
...
def print_best_summary(summary_df: pd.DataFrame) -> None:
    logger.info("\n===== BEST LLM BY HORIZON =====")
    for horizon in ["1d", "3d", "7d"]:
        ...
        best_rmse = sub.loc[sub["rmse_lstm_mean"].idxmin()]
        best_acc = sub.loc[sub["acc_lstm_mean"].idxmax()]
        best_f1 = sub.loc[sub["f1_lstm_mean"].idxmax()]

        logger.info(f"\nHorizon: {horizon}")
        logger.info(f"  Best RMSE provider: {best_rmse['provider']} ({best_rmse['rmse_lstm_mean']:.6f})")
        logger.info(f"  Best Accuracy provider: {best_acc['provider']} ({best_acc['acc_lstm_mean']:.6f})")
...
def main(results_dir: str = RESULTS_DIR, verbose: bool = False) -> None:
    ...
    for provider, path in DATASET_MAP.items():
        if not os.path.exists(path):
            raise FileNotFoundError(f"Missing dataset for provider '{provider}': {path}")

        logger.info(f"\n\n########## LOADING {provider.upper()} DATASET ##########")
        ...
        logger.info(f"Number of feature columns for {provider}: {len(feature_cols)}")

        for horizon_name, target_col in horizon_map.items():
            fold_df, summary_df, preds_df = run_provider_horizon(
                provider=provider,
                df=df,
    ...
    fold_path = os.path.join(results_dir, "fold_metrics_llm_compare.csv")
    summary_path = os.path.join(results_dir, "summary_llm_compare.csv")
    preds_path = os.path.join(results_dir, "predictions_llm_compare.csv")
    json_path = os.path.join(results_dir, "summary_llm_compare.json")
    ...
    logger.info("\n===== FINAL SUMMARY =====")
    logger.info(summary_results_df)

    print_best_summary(summary_results_df)
