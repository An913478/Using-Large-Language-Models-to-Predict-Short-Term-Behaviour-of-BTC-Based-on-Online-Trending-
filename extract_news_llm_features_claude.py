"""Extract daily LLM features from MarketAux daily text using Claude.

This module queries Anthropic's Claude API to extract structured features
from daily news text. It writes periodic checkpoints so the process can be
resumed. This operation is networked and not safe for automated smoke-tests.
"""
...
PROVIDER = "claude"
MODEL_NAME = os.getenv("ANTHROPIC_LLM_COMPARE_MODEL", "claude-sonnet-4-5")
MAX_ATTEMPTS = 4
CHECKPOINT_EVERY = 25


def call_claude(client: anthropic.Anthropic, date_str: str, daily_text: str, model_name: str) -> Dict:
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    """Parse CLI arguments for Claude LLM feature extraction."""
    p = argparse.ArgumentParser(description="Extract LLM features using Claude")
...
def main(argv: Optional[list[str]] = None) -> None:
    """Entry point for the Claude provider CLI."""
    ...
    for i, (_, row) in enumerate(todo_df.iterrows(), start=1):
        ...
        if should_skip_text(daily_text):
            ...
            features = call_claude(client, date_str, daily_text, args.model)
